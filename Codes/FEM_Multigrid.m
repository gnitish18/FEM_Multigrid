...
for i = 1:m
    j = i+1;
    t1 = ((j^4 + 3*i^4)/12) - ((j^3 + 2*i^3)/3) + ((j^2 + i^2)/2) - j*(i^3/3 - i^2 + i);
    t2 = ((3*j^4 + i^4)/12) - ((2*j^3 + i^3)/3) + ((j^2 + i^2)/2) - i*(j^3/3 - j^2 + j);
    term3(i) = term3(i) - 150/h * t1;
    term3(j) = term3(j) - 150/h * t2;
end

F = zeros(n,1);                     % Force matrix
x = 5;                              % Length of the beam
F(n) = -150*x + 150*x^2 - 50*x^3;   % Last term - slope
F = F + term3;
disp(Ah)
...
% for i = 1:m
%     j = i+1;
%     t1 = -50*i^3 + 12.5*j^4/h - i^4 + 150*i^2 - 50*j^3/h + 50*i^3/h +75*h;
%     t2 = 50*j^3 - 12.5*j^4/h + i^4 - 150*j^2 + 50*j^3/h - 50*i^3/h +75*h;
%     term3(i) = term3(i) + t1;
%     term3(j) = term3(j) + t2;
% end
% disp(term3);
